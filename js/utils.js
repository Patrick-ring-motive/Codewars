    ...
    try {
      if (arguments.length == 1) {
        try {
          ...
          return [].reduce.call(s, function(x, y) {
            return 1 * x + 1 * y
          }, 0);
        } catch (e) {}
      }
      ...
      return [].reduce.call(function(x, y) {
        return 1 * x + 1 * y
      }, 0);
    } catch (e) {
      return 0
...
Math.sum = function() {
  try {
    if (arguments.length == 1) {
      try {
        return [...arguments[0]].reduce((x, y) => +x + +y, 0);
      } catch (e) {}
    }
...
function t(x) {
  return Number ? Number(x) : 1 * x;
}

function n(x) {
  return BigInt ? BigInt(x) : parseFloat(x);
}
